...
const DashboardPreview = () => {
    return (
        <div className="w-full bg-[#fcfcfc] flex flex-col h-full min-h-[600px] text-gray-800 font-sans overflow-hidden select-none">
            ...
            <header className="h-[70px] bg-white border-b border-gray-100 flex items-center justify-between px-8 shrink-0">
                <div className="flex items-center gap-12">
                    <div className="flex items-center gap-2">
                        ...
                        <span className="text-gray-900 font-bold text-lg tracking-tight">EatGreet</span>
                    </div>
                    ...
                    <nav className="flex items-center gap-2">
                        {['Dashboard', 'Menu', 'Category', 'Order', 'Table', 'Sales'].map((item) => (
                            <button
                                key={item}
                                className={`px-5 py-2 rounded-full text-[13px] font-bold transition-all ${item === 'Dashboard' ? 'bg-black text-white shadow-sm' : 'text-gray-500 hover:bg-gray-50'
                                    }`}
                            >
                                {item}
                            </button>
            ...
            <div className="flex-1 overflow-y-auto p-10 space-y-8">
                <div>
                    ...
                    <p className="text-[11px] text-gray-400 font-medium mt-1.5 tracking-[0.1em] uppercase opacity-80">WELCOME BACK, ADMIN</p>
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-4 gap-8 items-start">
                    ...
                    <div className="lg:col-span-3 space-y-8">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                            {[
                                ...
                                <motion.div
                                    ...
                                    className="bg-white p-7 rounded-[2rem] border border-gray-100/50 shadow-[0_4px_20px_-1px_rgba(0,0,0,0.02)] flex items-center justify-between"
                                >
                                    <div className="flex items-center gap-5">
                                        <div className="w-12 h-12 bg-gray-50/50 rounded-2xl flex items-center justify-center border border-gray-100">
                                            <stat.icon className={`w-6 h-6 ${stat.color}`} />
                                        </div>
                                        <div>
                                            <div className="flex items-baseline">
                                                <h4 className="text-2xl font-bold font-['Urbanist'] tracking-tight text-gray-900">{stat.value}</h4>
                                                {stat.sub && <span className="text-sm font-semibold text-gray-300 ml-1">{stat.sub}</span>}
                                            </div>
                                            <p className="text-[10px] font-medium text-gray-400 uppercase tracking-widest mt-1 opacity-80">{stat.label}</p>
                                        </div>
                                    </div>
                                </motion.div>
                            ))}
                        ...
                        <motion.div
                            ...
                            className="bg-white p-8 rounded-[2.5rem] border border-gray-100 shadow-[0_8px_30px_rgb(0,0,0,0.02)] min-h-[420px] flex flex-col relative"
                        >
                            <div className="flex justify-between items-start mb-12">
                                <div>
                                    <h5 className="text-lg font-bold text-gray-900">Sales Analytics</h5>
                                    <p className="text-[10px] text-gray-400 font-medium mt-1.5 tracking-[0.15em] uppercase opacity-80">TODAY BREAKDOWN</p>
                                </div>
                                <div className="w-10 h-10 rounded-full bg-gray-50 flex items-center justify-center text-gray-300 border border-gray-100 cursor-pointer hover:text-[#FD6941] transition-colors group">
