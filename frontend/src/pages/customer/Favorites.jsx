    ...
    return (
        <div className="bg-white min-h-[80vh]">
            <div className="mb-6 flex items-center gap-3">
                ...
                <h1 className="text-2xl font-normal text-gray-800">Saved Items</h1>
            ...
            {favoriteItems.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-20 text-center">
                    ...
                    <h2 className="text-xl font-normal text-gray-800 mb-2">No Saved Items</h2>
                    <p className="text-gray-500 mb-6 max-w-xs mx-auto">Items you like will appear here for quick access.</p>
                    <Link to="../menu" className="bg-black text-white px-8 py-3 rounded-full font-normal shadow-lg hover:scale-105 transition-transform">
                        Explore Menu
                    </Link>
                </div>
            ) : (
                        ...
                        <div key={item._id || item.id} className="bg-white rounded-[2.5rem] p-4 shadow-sm border border-gray-100 hover:shadow-lg transition-all group relative overflow-hidden">
                            ...
                            <div className="px-2 pb-2">
                                <div className="flex justify-between items-start mb-2">
                                    <div>
                                        <h3 className="text-lg font-normal text-gray-800 leading-tight mb-1">{item.name}</h3>
                                    ...
                                    <div className="text-right">
                                        <span className="block text-xl font-normal text-gray-800">{activeSymbol}{item.price}</span>
                                    </div>
                                </div>
                                    ...
                                    {cart[item._id || item.id] ? (
                                        <div className="flex items-center gap-4 bg-black text-white px-2 py-2 rounded-full shadow-lg w-full max-w-[140px] justify-between">
                                            ...
                                            <button
                                                ...
                                                className="w-8 h-8 rounded-full bg-[#FD6941] flex items-center justify-center hover:bg-[#FD6941]/90 transition"
                                            >
                                        ...
                                        <button
                                            onClick={() => addToCart(item)}
                                            className="w-full bg-black text-white py-3 rounded-full font-normal shadow-lg flex items-center justify-center gap-2 hover:bg-gray-800 active:scale-95 transition-all"
                                        >
                                            Add to Cart
                                        </button>
                                    )}
